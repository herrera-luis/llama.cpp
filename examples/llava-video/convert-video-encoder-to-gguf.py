...
def should_skip_tensor(name: str, has_text: bool, has_vision: bool, has_llava: bool) -> bool:
    ...
    if name.startswith("t") and not has_text:
     ...
     # Skip tensors containing 'lora_A' or 'lora_B'
...
def get_tensor_name(name: str) -> str:
    ...
    return name.replace("text_model", "t").replace("vision_model", "v").replace("encoder.layers", "blk").replace("encoder.base_model.model.layers", "blk").replace("embeddings.", "").replace("_proj", "").replace("self_attn.", "attn_").replace("layer_norm", "ln").replace("layernorm", "ln").replace("mlp.fc1", "ffn_down").replace("mlp.fc2", "ffn_up").replace("embedding", "embd").replace("final", "post").replace("layrnorm", "ln")
...
model = LanguageBindVideo.from_pretrained(dir_model)
processor = None
...
if has_vision_encoder:
    ...
    for name, data in projector.items():
        ...
        fout.add_tensor(f"clip.vision.{name}", data)
